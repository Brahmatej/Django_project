...
class StateCity(APIView):
    def get(request, state=None, city=None):
        hospitals=hospital.objects.filter(state__iexact=state).filter(city__iexact=city)
        serializer=HospitalSerializer(hospitals, many=True)
        return Response(serializer.data)
class StateCityPincode(APIView):
    def get(request, state=None, city=None, pincode=None):
